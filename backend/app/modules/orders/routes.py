...
def create_order(order_data: schemas.OrderCreate, db: Session = Depends(get_db)):
    ...
    order = Order(
        ...
        weight=int(float(order_data.weight) * 1000) if (order_data.weight is not None and 0 < float(order_data.weight) < 10.0) else (int(order_data.weight) if (order_data.weight is not None and int(order_data.weight) > 0) else 500),
