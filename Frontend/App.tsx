...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Listen for global auth errors (401)
    const handleAuthError = () => {
      setIsAuthenticated(false);
      navigate('/'); // Redirect to root if auth fails
    };
  ...
  // Redirect to / if not authenticated (Root is now Login)
  useEffect(() => {
  ...
  // Helper to check permission (case-insensitive)
      ...
      < main className="flex-1 flex flex-col min-w-0 overflow-hidden" >
        ...
        < div className="flex-1 overflow-auto p-4 sm:p-6 lg:p-8" >
          <Routes>
            ...
            <Route path="/notifications" element={<ProtectedRoute permission="Settings"><SettingsView activeView="set-notifications" /></ProtectedRoute>} />
          </Routes>
        </div >
      </main >
    </div >
